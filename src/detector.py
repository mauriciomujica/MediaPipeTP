...
import numpy as np
from .config import FACE_MESH_CONFIG, LANDMARK_COLOR, LANDMARK_RADIUS, LANDMARK_THICKNESS, VISUALIZATION_MODES
from .utils import calcular_apertura_boca, calcular_apertura_ojos, calcular_inclinacion_cabeza
        ...
        if resultados.multi_face_landmarks:
            ...
            if visualization_mode == "Puntos simples":
                ...
                self.mp_drawing.draw_landmarks(
                    ...
                    landmark_drawing_spec=self.mp_drawing.DrawingSpec(color=LANDMARK_COLOR, thickness=LANDMARK_THICKNESS, circle_radius=LANDMARK_RADIUS),
                    connection_drawing_spec=self.mp_drawing.DrawingSpec(color=LANDMARK_COLOR, thickness=1)
                )
            elif visualization_mode == "Heatmap de densidad de puntos":
                imagen_con_puntos = self._create_heatmap(imagen_con_puntos, rostro, ancho, alto)

            # Calcular métricas
            info["apertura_boca"] = calcular_apertura_boca(rostro, alto, ancho)
            info["apertura_ojos"] = calcular_apertura_ojos(rostro, alto, ancho)
            info["inclinacion_cabeza"] = calcular_inclinacion_cabeza(rostro, alto, ancho)
